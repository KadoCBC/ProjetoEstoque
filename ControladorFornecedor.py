...
class ControladorFornecedor:
    ...
    def lista_fornecedores(self):
        if len(self.__fornecedores) == 0:
            ...
            return None
        self.__tela_fornecedor.mostrar_mensagem('------------------------------------------')
        self.__tela_fornecedor.mostrar_mensagem('LISTA DE FORNECEDORES')
        for fornecedor in self.__fornecedores:
            self.__tela_fornecedor.mostrar_fornecedor({"nome": fornecedor.nome, "id": fornecedor.id})
    ...
    def mostrar_lista_enderecos(self):
        ...
        if len(self.fornecedores) == 0:
            self.__tela_fornecedor.mostrar_mensagem('Lista de Fornecedores está vazia')
            return None  
        elif len(self.__enderecos) == 0:
            self.__tela_fornecedor.mostrar_mensagem("Não há endereços cadastrados")
            print()
        self.__tela_fornecedor.mostrar_mensagem('LISTA DE ENDEREÇOS')
        print()
        for id_fornecedor, endereco in self.__enderecos.items():
            mensagem = (f"Fornecedor ID: {id_fornecedor}\n"
                        f"Rua: {endereco.rua}\n"
